...
class l(LSM):
    def __init__(self, size=64, threshold=1.0, beta=0.9):
...
class custom_layer(Layer):
    def __init__(self):
        super().__init__(name="custom_layer", size=None, layer_type="custom torch")
...
NODE_INSTANCES = {
    "l": l_instance,
    "l1": l1_instance,
    "answers": answers_instance,
    "custom_layer": custom_layer_instance,
}
...
simulation_outputs = []
simulation_config = RDLDatasetConfig(mode="rate", sim_steps=100, encode_steps=32, batch_size=8)
...
for _, row in simulation_rows.iterrows():
    ...
    sample_out = _run_single_sample(encoded, NODE_INSTANCES, GRAPH_CONNECTIONS, explicit_entry="l1")
